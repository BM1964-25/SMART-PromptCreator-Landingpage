# SMART PromptCreator Landingpage

Eigenständige Landingpage für die browserbasierte App SMART PromptCreator.

## Stack

- React
- Vite
- TypeScript
- Tailwind CSS

## Entwicklung

```bash
npm install
npm run dev
```

Lokale URL:

```text
http://127.0.0.1:8172/SMART-PromptCreator-Landingpage/
```

## Build

```bash
npm run build
```

## Hinweise

- Hero-Screenshot: `public/hero-app-screenshot.png`
- App-Logo: `public/app-logo.svg`
- Vite Base-Pfad: `/SMART-PromptCreator-Landingpage/`

Wenn das GitHub-Repository anders heißt, muss der `base`-Wert in `vite.config.ts` entsprechend angepasst werden.
